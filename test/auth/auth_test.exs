defmodule Ptolemy.AuthTest do
  ...
  @gcp_svc1_with_vault_perm %{
    ...
    "client_x509_cert_url" => "https://www.googleapis.com/dont/bother/bots/becuase/this-is-a-fake-svc-acc@project-id.iam.gserviceaccount.com"
  }
  ...
  @result_IAP_bearer  {"Authorization", "Bearer 98a4c7ab98a4c7ab98a4c7ab"}
    ...
    mock(fn
      ...
      %{method: :post, url: "#{@google_auth_url}/oauth2/v4/token"} ->
        json(%{
          ...
          "access_token" => "ya29.c.ifkjshdflkjsdhlfkjsrhgeurghoiRANDOMDATASCREWOFFBOTSosdifjsldkfasd",
          "expires_in" => 3600,
          "token_type" => "Bearer"
          },
        status: 200)
   
      %{method: :post, url: "#{@iam_auth_url}/v1/projects/some-id-of-a-fake-project/serviceAccounts/this-is-a-fake-svc-acc@project-id.iam.gserviceaccount.com:signJwt"} ->
        json(%{
  ...
  test "Vault authentication success via approle, with IAP disabled/not enabled" do
    assert @result_NOIAP == Ptolemy.Auth.authenticate(:Approle, @vurl, %{secret_id: "test", role_id: "test"}, [])
  end 

  test "Vault authentication success via approle, with IAP enabled and Bearer token re-used" do
    assert @result_NOIAP == Ptolemy.Auth.authenticate(:Approle, @vurl, %{secret_id: "test", role_id: "test"}, {"Authorization", "Bearer 98a4c7ab98a4c7ab98a4c7ab"})
  end

  test "Vault authentication success via approle, with IAP enabled and Bearer token generated" do
    assert @result_IAP == Ptolemy.Auth.authenticate(:Approle, @vurl, %{secret_id: "test", role_id: "test"}, @iap_opt)
  end

  test "Vault authentication success via GCP, with IAP disabled/not enabled" do
    assert @result_NOIAP == Ptolemy.Auth.authenticate(:GCP, @vurl, %{gcp_svc_acc: @gcp_svc1_with_vault_perm, vault_role: "test", exp: 3000}, [])
  end

  test "Vault authentication success via GCP, with IAP enabled and Bearer token re-used" do
    assert @result_NOIAP == Ptolemy.Auth.authenticate(:GCP, @vurl, %{gcp_svc_acc: @gcp_svc1_with_vault_perm, vault_role: "test", exp: 3000}, {"Authorization", "Bearer 98a4c7ab98a4c7ab98a4c7ab"})
  end
  ...
  test "Vault authentication success via GCP, with IAP enabled and Bearer token generated with the same GCP service account being used for Vault" do
    assert @result_IAP == Ptolemy.Auth.authenticate(:GCP, @vurl, %{gcp_svc_acc: @gcp_svc1_with_vault_perm, vault_role: "test", exp: 3000}, @iap_opt_reuse)
  end
  ...
  test "Vault authentication success via GCP, with IAP enabled and Bearer token generated with two seperate GCP service accounts" do
    assert @result_IAP == Ptolemy.Auth.authenticate(:GCP, @vurl, %{gcp_svc_acc: @gcp_svc1_with_vault_perm, vault_role: "test", exp: 3000}, @iap_opt)
  end
end