defmodule Ptolemy.Engines.PKI.Engine do
  ...
  """
  @spec create_role(Tesla.Client.t(), String.t(), map()) ::
  ...
  """
  @spec generate_secret(Tesla.Client.t(), String.t(), String.t(), map()) ::
  ...
  def generate_secret(client, path, common_name, payload \\ %{}) do
    ...
    with {:ok, resp} <- Tesla.post(client, "#{path}", payload) do
      case {resp.status, resp.body} do
        ...
        {status, _} ->
          {:error,
           "Could not generate PKI certificate from the role in remote vault server. Error code: #{
             status
           }"}
      end
    end
  ...
  """
  @spec revoke_cert(Tesla.Client.t(), String.t(), String.t(), map()) ::
          {:ok, String.t()} | {:error, String.t()}
  def revoke_cert(client, path, serial_number, payload \\ %{}) do
    ...
    with {:ok, resp} <- Tesla.post(client, "#{path}", payload) do
      case {resp.status, resp.body} do
        ...
        {status, _} ->
          {:error,
           "Could not revoke PKI certificate from the remote vault server. Error code: #{status}"}
      end
    end
  end
  ...
  """
  @spec revoke_role(Tesla.Client.t(), String.t()) :: {:ok, String.t()} | {:error, String.t()}
  def revoke_role(client, path) do
    with {:ok, resp} <- Tesla.delete(client, "#{path}") do
      case {resp.status, resp.body} do
        ...
        {status, _} ->
          {:error,
           "Could not revoke PKI role from the remote vault server. Error code: #{status}"}
      end
    end
  end
end
