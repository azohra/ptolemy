defmodule Ptolemy.Engines.KV.Engine do
  ...
  """
  @spec read_secret(Tesla.Client.t(), String.t(), list(integer)) :: {:ok | :error, String.t()}
  ...
  """
  @spec create_secret(Tesla.Client.t(), String.t(), map()) :: {:ok | :error, String.t()}
  ...
  """
  @spec delete(Tesla.Client.t(), String.t(), list(integer)) :: {:ok | :error, String.t()}
  def delete(client, path, vers) do
    ...
    with {:ok, resp} <- Tesla.post(client, "#{path}", payload) do
      case {resp.status, resp.body} do
        ...
        {status, _ } ->
          {:error, "Could not delete version(s) of secret in remote vault server. Error code: #{status}"}
      end
    end
  end
  ...
  If a 403 response is received, please check your ACL policy on vault
  """
  @spec destroy(Tesla.Client.t(), String.t(), list(integer)) :: {:ok | :error, String.t()}
  def destroy(client, path, vers) do
    ...
    with {:ok, resp} <- Tesla.post(client, "#{path}", payload) do
      case {resp.status, resp.body} do
        ...
        {status, _ } ->
          {:error, "Could not destroy version(s) of secret in remote vault server. Error code: #{status}"}
      end
    end
  end
end
